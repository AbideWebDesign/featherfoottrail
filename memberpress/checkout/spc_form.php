...
<div class="mp_wrapper wrapper-sm">
	<div class="p-4 bg-white">
		...
		<h3 class="text-primary"><strong>Subscription</strong></h3>
		
		<p class="text-primary mb-0">Monthly Subscription: <?php MeprProductsHelper::display_invoice( $product, $mepr_coupon_code ); ?></p>
	...
	<div class="bg-brown p-4">
	    ...
	    <?php if($product->adjusted_price($mepr_coupon_code) > 0.00 || !empty($product->plan_code)): ?>
	      <?php if($mepr_options->coupon_field_enabled): ?>
	        ...
	        <div class="mb-3">Have a Coupon?</div>
	        
	        <div class="mp-form-row mepr_coupon mepr_coupon_<?php echo $product->ID; ?> form-group">
	          <div class="mp-form-label">
	          ...
	          <input type="text" id="mepr_coupon_code<?php echo $unique_suffix; ?>" class="mepr-form-input mepr-coupon-code form-control" name="mepr_coupon_code" placeholder="Coupon Code" value="<?php echo (isset($mepr_coupon_code))?esc_attr(stripslashes($mepr_coupon_code)):''; ?>" data-prdid="<?php echo $product->ID; ?>" />
	        </div>
	      <?php else: ?>
	...
	<div class="bg-white p-4">
	    ...
	    <?php if($product->adjusted_price($mepr_coupon_code) <= 0.00) { ?>
	      <div class="mepr-transaction-invoice-wrapper" style="padding-top:10px">
	        <span class="mepr-invoice-loader mepr-hidden">
	          <img src="<?php echo includes_url('js/thickbox/loadingAnimation.gif'); ?>" title="<?php _ex('Loading icon', 'ui', 'memberpress'); ?>" width="100" height="10" />
	        </span>
